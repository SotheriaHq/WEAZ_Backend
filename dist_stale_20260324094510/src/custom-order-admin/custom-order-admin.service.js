...
exports.CustomOrderAdminService = void 0;
...
let CustomOrderAdminService = class CustomOrderAdminService {
    ...
    async reviewBasis(id, dto, adminUserId) {
        const basis = await this.prisma.customFabricRuleBasis.findUnique({ where: { id } });
        if (!basis) {
            throw new common_1.NotFoundException('Custom fabric rule basis not found');
        }
    ...
    async listBases(query) {
        const where = {
            status: client_1.CustomFabricRuleBasisStatus.APPROVED_GLOBAL,
        };
        const items = await this.prisma.customFabricRuleBasis.findMany({
            where,
    ...
    async createBasis(dto, _adminUserId) {
        const measurementKeys = Array.from(new Set((dto.measurementKeys ?? []).map((key) => String(key).trim()).filter(Boolean)));
        if (measurementKeys.length === 0) {
            throw new common_1.BadRequestException('At least one measurement key is required for a fabric rule basis');
        }
    ...
    async updateBasis(id, dto) {
        ...
        if (!existing || existing.status !== client_1.CustomFabricRuleBasisStatus.APPROVED_GLOBAL) {
            throw new common_1.NotFoundException('Global custom fabric rule basis not found');
        }
    ...
    async deleteBasis(id) {
        ...
        if (existing.configurations.length > 0) {
            throw new common_1.BadRequestException('Cannot delete this basis because it is already used by active configurations');
        }
    ...
    async decideExceptionReview(customOrderId, eventId, dto, adminUserId) {
        ...
        if (!event) {
            throw new common_1.NotFoundException('Exception review request not found');
        }
        const payload = (event.payloadJson ?? {});
        if (payload.kind !== 'EXCEPTION_REVIEW_REQUEST') {
            throw new common_1.BadRequestException('Invalid exception review request payload');
        }
        ...
        const updatedOrder = await this.prisma.$transaction(async (tx) => {
            ...
            return tx.customOrder.update({
                ...
                data: {
                    internalPriceBreakdownJson: nextBreakdown,
                    timelineEvents: {
                        create: {
                            actorType: client_1.CustomOrderActorType.ADMIN,
                            actorId: adminUserId,
                            eventType: 'ADMIN_ESCALATED',
                            payloadJson: {
                                ...
                                decision: nextStatus,
                                rationale: dto.rationale.trim(),
                                approvedQuoteTotal: nextStatus === 'APPROVED' ? dto.approvedQuoteTotal ?? null : null,
                            },
                        },
                    },
                },
    ...
    async getSummary() {
        ...
        const [activeOrders, refundInProgress, deliveredAwaitingConfirmation, openDisputes, acceptanceSlaRisk, acceptanceTimeouts, staleOrderIds, disputeOrders, rejectedOrders,] = await this.prisma.$transaction([
            ...
            this.prisma.customOrder.count({
                where: { status: client_1.CustomOrderStatus.REFUND_IN_PROGRESS },
            }),
            this.prisma.customOrder.count({
                where: { status: client_1.CustomOrderStatus.DELIVERED_PENDING_BUYER_CONFIRMATION },
            }),
            this.prisma.customOrderDispute.count({
                where: {
                    status: {
                        in: [
                            client_1.CustomOrderDisputeStatus.OPEN,
                            client_1.CustomOrderDisputeStatus.BRAND_RESPONDED,
                            client_1.CustomOrderDisputeStatus.ADMIN_REVIEW,
                        ],
                    },
                },
            }),
            ...
            this.prisma.customOrder.findMany({
                where: { status: client_1.CustomOrderStatus.REJECTED_BY_BRAND },
                select: { brandId: true },
            }),
        ]);
        ...
        const topRiskBrandIds = [...riskByBrand.entries()]
            .sort((left, right) => right[1].stale + right[1].disputes + right[1].rejections -
    ...
    async getRiskDashboard(query) {
        ...
        const [periodOrders, periodAnalytics, staleEvents, acceptanceSlaRiskOrders, acceptanceTimeoutOrders] = await Promise.all([
            this.prisma.customOrder.findMany({
                where: {
                    ...orderWhere,
                    createdAt: { gte: since },
        ...
        const rushExceptionStatuses = new Set([
        ...
        const topBrandIds = [...brandRiskMap.entries()]
            .map(([brandId, metrics]) => ({
            brandId,
            riskScore: metrics.brandRejections * 4 +
    ...
    async listRefundReviews(query) {
        ...
        const refundEvents = attemptIds.length
            ...
            : [];
        const latestRefundEvents = new Map();
    ...
    async releaseEligibleLedgerAllocations(dto, adminUserId) {
        const now = new Date();
            ...
            await this.prisma.$transaction(async (tx) => {
                ...
                await tx.payout.create({
                    data: {
                        ...
                        amount: new client_1.Prisma.Decimal(group.totalAmount.toFixed(2)),
                ...
                if (reserved.count !== group.allocationIds.length) {
                    throw new common_1.BadRequestException('CUSTOM_ORDER_PAYOUT_BATCH_RESERVATION_FAILED');
                }
    ...
    async updateDispute(id, dto, adminUserId) {
        const dispute = await this.prisma.customOrderDispute.findUnique({ where: { id } });
        if (!dispute) {
            throw new common_1.NotFoundException('Custom-order dispute not found');
        }
    ...
    async updateRetentionHold(id, dto, adminUserId) {
        const order = await this.prisma.customOrder.findUnique({
            where: { id },
            select: {
                id: true,
        ...
        if (!dto.clear) {
            if (!dto.holdType) {
                throw new common_1.BadRequestException('Retention hold type is required');
            }
            if (!dto.reason?.trim()) {
                throw new common_1.BadRequestException('Retention hold reason is required');
            }
        }
        const updated = await this.prisma.customOrder.update({
            ...
            data: dto.clear
                ...
                : {
                    retentionHoldType: dto.holdType,
    ...
    async remindBrand(id, dto, adminUserId) {
        const order = await this.prisma.customOrder.findUnique({
            where: { id },
            include: {
                brand: { select: { id: true, name: true, ownerId: true } },
        ...
        if (!order.brand.ownerId) {
            throw new common_1.BadRequestException('Brand owner could not be resolved for this order');
        }
        await this.prisma.customOrderTimelineEvent.create({
            data: {
    ...
    async flagRisk(id, dto, adminUserId) {
        const order = await this.prisma.customOrder.findUnique({
            where: { id },
    ...
    async escalateRefundReview(id, dto, adminUserId) {
        const order = await this.prisma.customOrder.findUnique({
            where: { id },
            select: {
                id: true,
        ...
        if (!eligibleStatuses.has(order.status)) {
            throw new common_1.BadRequestException('Custom order cannot be moved to refund review from its current state');
        }
        const updated = await this.prisma.customOrder.update({
            where: { id },
            data: {
    ...
    async cancelPaidOrder(id, dto, adminUserId) {
        const order = await this.prisma.customOrder.findUnique({
            where: { id },
            select: {
                id: true,
        ...
        if (order.paymentStatus !== client_1.PaymentStatus.PAID) {
            throw new common_1.BadRequestException('Only fully paid custom orders can be cancelled from admin');
        }
        ...
        if (!cancellableStatuses.has(order.status)) {
            throw new common_1.BadRequestException('Custom order cannot be cancelled from its current state');
        }
        ...
        const updated = await this.prisma.$transaction(async (tx) => {
            ...
            await tx.customOrderLedgerAllocation.updateMany({
                where: {
                    ...
                    status: {
                        in: [
                            client_1.CustomOrderLedgerAllocationStatus.HELD,
                            client_1.CustomOrderLedgerAllocationStatus.PAYOUT_ELIGIBLE,
                        ],
                    },
                },
                data: {
        ...
        const recipientIds = [order.buyerId, order.brand?.ownerId].filter((recipientId) => Boolean(recipientId));
...
exports.CustomOrderAdminService = CustomOrderAdminService;
exports.CustomOrderAdminService = CustomOrderAdminService = __decorate([
...
//# sourceMappingURL=custom-order-admin.service.js.map