...
export class CommentsV2Controller {
  ...
  createForPost(@Param('postId') postId: string, @Body() dto: CreateCommentV2Dto, @Req() req: any) {
  ...
  createForCollection(@Param('collectionId') collectionId: string, @Body() dto: CreateCommentV2Dto, @Req() req: any) {
    return this.service.createForTarget('COLLECTION', collectionId, req.user.id, dto);
  }

  // Create comment for COLLECTION_MEDIA
  @Post('collections/media/:mediaId/comments')
  createForMedia(@Param('mediaId') mediaId: string, @Body() dto: CreateCommentV2Dto, @Req() req: any) {
    return this.service.createForTarget('COLLECTION_MEDIA', mediaId, req.user.id, dto);
  }

  // List comments for a target (top-level only; preload latest 2 replies)
  @Get('posts/:postId/comments')
  listForPost(@Param('postId') postId: string, @Query() q: ListQueryDto, @Req() req: any) {
    return this.service.listForTarget('POST', postId, req.user?.id, q);
  }

  @Get('collections/:collectionId/comments')
  listForCollection(@Param('collectionId') id: string, @Query() q: ListQueryDto, @Req() req: any) {
    return this.service.listForTarget('COLLECTION', id, req.user?.id, q);
  }

  @Get('collections/media/:mediaId/comments')
  listForMedia(@Param('mediaId') id: string, @Query() q: ListQueryDto, @Req() req: any) {
    return this.service.listForTarget('COLLECTION_MEDIA', id, req.user?.id, q);
  }

  // Replies
  @Get('comments/:id/replies')
  getReplies(@Param('id') id: string, @Query() q: ListQueryDto, @Req() req: any) {
  ...
  toggleLike(@Param('id') id: string, @Req() req: any) {
    const clientEventId = typeof req.headers['x-client-event-id'] === 'string' ? req.headers['x-client-event-id'] : undefined;
