...
import { CustomOrderCheckoutStatus, CustomOrderStatus, PaymentStatus, SizingMode } from '@prisma/client';
import type {
  BagDuplicateState,
...
export class BagValidationService {
  validateStandardBagInput(input: StandardBagValidationInput) {
    ...
    const colors = Array.isArray(input.product.colors) ? input.product.colors : [];
    ...
    if (input.resultingQuantity > 99) {
      throw new BadRequestException('Cart quantity limit exceeded for this item (max 99)');
    }
    ...
    if (input.product.trackInventory && !input.product.allowBackorders) {
      if (selectedVariant) {
        ...
        if (available < input.resultingQuantity) {
          throw new BadRequestException(`Only ${available} items available for the selected variant`);
        }
        ...
        if (available < input.resultingQuantity) {
          throw new BadRequestException(`Only ${available} items available in size ${input.selectedSize}`);
        }
      } else if (Number(input.product.totalStock || 0) < input.resultingQuantity) {
        throw new BadRequestException(`Only ${Number(input.product.totalStock || 0)} items available`);
      }
    }
  ...
  classifyDuplicateState(input: {
    ...
    const checkoutSessions = Array.isArray(input.checkoutSessions) ? input.checkoutSessions : [];
    const customOrders = Array.isArray(input.customOrders) ? input.customOrders : [];
    ...
    if (customOrders.some((order) => order.status === CustomOrderStatus.COMPLETED)) {
      classifications.push(
        input.completedPolicy === 'BLOCK_REPEAT' ? 'COMPLETED_BLOCKED' : 'COMPLETED_ALLOWED',
      );
    }
  ...
  private validateRtwPlusFittingsPayload(input: StandardBagValidationInput) {
    const sizingMode = this.normalizeSizingMode(input.sizingMode ?? input.product.sizingMode);
    if (sizingMode !== SizingMode.RTW_PLUS_FITTINGS) return;

    const productRequiredKeys = this.normalizeKeys(input.product.customMeasurementKeys);
    ...
    const providedKeys = this.extractProvidedMeasurementKeys(input.sizeFitData ?? null);
    const missingKeys = requiredMeasurementKeys.filter((key) => !providedKeys.includes(key));
  ...
  private extractProvidedMeasurementKeys(sizeFitData: Record<string, unknown> | null): string[] {
  ...
  private reasonForClassifications(classifications: CustomDuplicateClass[]): string | null {
    if (classifications.includes('IN_BAG')) return 'CUSTOM_ORDER_DUPLICATE_IN_BAG';
    if (classifications.includes('SUBMITTED_UNPAID')) return 'CUSTOM_ORDER_SUBMITTED_UNPAID_DUPLICATE';
    if (classifications.includes('PAID_ACTIVE')) return 'CUSTOM_ORDER_PAID_ACTIVE_DUPLICATE';
    if (classifications.includes('COMPLETED_BLOCKED')) return 'CUSTOM_ORDER_COMPLETED_DUPLICATE';
    return null;
  }
}
