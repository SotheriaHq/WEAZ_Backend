...
const KEY_DESCRIPTIONS: Record<string, string> = {
  ...
  'upload.maxSize.brandVerification':'Max file size for brand verification docs (bytes)',
...
export class SystemConfigService {
  ...
  async getUploadLimits(): Promise<Record<string, number>> {
    ...
    for (const key of Object.keys(DEFAULT_FILE_SIZE_LIMITS)) {
      ...
      limits[key] = Number.isFinite(val) && val > 0 ? val : DEFAULT_FILE_SIZE_LIMITS[key];
    }
    return limits;
  }
  ...
  async bulkUpdate(
    ...
    for (const entry of entries) {
      results.push(await this.updateConfig(entry.key, entry.value, actorId, req));
    }
    return results;
  }
}
