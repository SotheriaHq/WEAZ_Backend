...
@Module({
  imports: [
    ...
    SystemConfigModule,
    // MessagingSideEffectsService injects the realtime gateway with @Optional(). Without
    // it in scope Nest resolved it to `undefined`, so EVERY messaging realtime
    // emit — message.created / thread.updated / message.read — was a silent
    // no-op. Imported, never re-provided: see RealtimeModule.
    RealtimeModule,
  ],
  controllers: [
