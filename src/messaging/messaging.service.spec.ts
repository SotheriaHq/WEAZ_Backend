...
describe('MessagingService', () => {
  const buildService = (overrides?: {
    ...
    const access = {
      ...
      resolveActorThreadRole: jest
        .fn()
        .mockResolvedValue(MessageParticipantRole.BRAND_OWNER),
    } as any;
  ...
  it('filters moderated messages for buyer thread listing', async () => {
    ...
    query.getMessages.mockResolvedValue({
      items: [],
      hasNextPage: false,
      endCursor: null,
    });

    await service.listCustomOrderMessagesForBuyer('buyer_1', 'co_1', {
      limit: 20,
    });
  ...
  it('re-checks context status in transaction and blocks send on race to read-only', async () => {
    ...
    await expect(
      service.sendCustomOrderMessageForBuyer(
        ...
        'co_1',
        {
          clientMessageId: 'fd6dc77c-8ce8-4c5d-a44e-3566ea8b7f6f',
          bodyText: 'hello',
        },
        'idem-1',
      ),
  ...
  it('returns bulk summaries keyed by requested context ids for buyer custom orders', async () => {
    ...
    prisma.customOrder.findMany = jest
      .fn()
      .mockResolvedValue([{ id: 'co_1' }, { id: 'co_2' }]);
    ...
    const result = await service.getBulkSummariesForCustomOrdersBuyer(
      'buyer_1',
      {
  ...
  it('returns aggregate unread message count from messaging read state only', async () => {
    ...
    await expect(
      service.getUnreadMessageCountForActor('user_1'),
    ).resolves.toEqual({
      unreadCount: 7,
    });
  ...
  it('resolves messageId to an authorized conversation route', async () => {
    ...
    await expect(
      service.resolveConversationForActor('buyer_1', {
        messageId: 'message_1',
      }),
  ...
  it('does not resolve another user conversation by messageId', async () => {
    ...
    prisma.message.findUnique.mockResolvedValue({ threadId: 'thread_1' });
    access.assertCanReadThread.mockRejectedValue(
      new ForbiddenException('Thread access denied'),
    );
    prisma.messageThreadParticipant.findUnique.mockResolvedValue(null);

    await expect(
      service.resolveConversationForActor('intruder_1', {
        messageId: 'message_1',
      }),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });
