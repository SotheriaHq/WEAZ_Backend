...
describe('catalog domain boundary contracts', () => {
  it('keeps product DTO collectionId scoped to StoreCollection membership', () => {
    ...
    expect(productDto).toContain('collectionId means optional StoreCollection membership');
  ...
  it('keeps product service out of legacy design initialize/finalize flows', () => {
    ...
    expect(storeService).toContain('collectionId is only StoreCollection membership');
  });

  it('documents collection DTO compatibility instead of claiming product ownership', () => {
    const collectionProductsDto = source('src/collections/dto/collection-products.dto.ts');
    const extendedDto = source('src/collections/dto/collection-extended.dto.ts');
    const createCollectionDto = source('src/collections/dto/create-collection.dto.ts');

    expect(collectionProductsDto).toContain('STORE_COLLECTION_GROUPING');
    expect(collectionProductsDto).toContain('do not make collection own product inventory');
    expect(extendedDto).toContain('Collection remains a container');
    expect(createCollectionDto).toContain('LEGACY_COMPAT_COLLECTION_BACKED_DESIGN');
  });
});
