import { resolveCatalogEntityType, withCatalogEntityType } from './catalog-read.mapper';
...
describe('catalog read entity mapper', () => {
  ...
  it('maps legacy collection-backed design records to DESIGN', () => {
    ...
    expect(resolveCatalogEntityType({ domain: 'DESIGN', id: 'collection-1' })).toBe('DESIGN');
  });

  it('maps product/store product records to PRODUCT', () => {
    expect(resolveCatalogEntityType({ sourceType: 'STORE_PRODUCT', id: 'product-1' })).toBe('PRODUCT');
    expect(resolveCatalogEntityType({ id: 'product-2', price: 1200, totalStock: 4 })).toBe('PRODUCT');
  });

  it('maps store collection/grouping records to COLLECTION', () => {
    expect(resolveCatalogEntityType({ sourceType: 'STORE_COLLECTION', id: 'store-collection-1' })).toBe('COLLECTION');
  ...
  it('does not guess ambiguous records unless a fallback is supplied', () => {
    expect(resolveCatalogEntityType({ id: 'unknown-1', title: 'Untitled' })).toBeNull();
    expect(resolveCatalogEntityType({ id: 'unknown-1', title: 'Untitled' }, 'DESIGN')).toBe('DESIGN');
  });
