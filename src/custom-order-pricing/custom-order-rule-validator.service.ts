...
export class CustomOrderRuleValidatorService {
  normalizeRules(
    ...
    if (!Array.isArray(rules) || rules.length === 0) {
      throw new BadRequestException(
        'At least one custom fabric rule is required',
      );
    }
    ...
    const normalized = rules.map((rule) => {
      ...
      if (!Number.isFinite(outputYards) || outputYards <= 0) {
        throw new BadRequestException(
          'Each custom fabric rule must have a positive yard output',
        );
      }
  ...
  validateRules(rules: NormalizedCustomFabricRule[]) {
    ...
    if (fallbackRules.length !== 1) {
      throw new BadRequestException(
        'Exactly one fallback fabric rule is required',
      );
    }
    ...
    for (const rule of rules) {
      if (!Number.isInteger(rule.priority) || rule.priority < 1) {
        throw new BadRequestException(
          'Custom fabric rule priority must be an integer greater than 0',
        );
      }

      if (seenPriorities.has(rule.priority)) {
        throw new BadRequestException(
          'Custom fabric rules must use unique priorities',
        );
      }
      ...
      if (rule.isFallback && rule.conditions.length > 0) {
        throw new BadRequestException(
          'Fallback fabric rule cannot define conditions',
        );
      }
      ...
      if (seenSignatures.has(signature)) {
        throw new BadRequestException(
          'Duplicate custom fabric rules are not allowed',
        );
      }
  ...
  private normalizeCondition(
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      throw new BadRequestException(
        `Invalid rule condition for measurement key ${key}`,
      );
    }
    ...
    if (min != null && !Number.isFinite(min)) {
      throw new BadRequestException(
        `Invalid minimum rule value for measurement key ${key}`,
      );
    }
    if (max != null && !Number.isFinite(max)) {
      throw new BadRequestException(
        `Invalid maximum rule value for measurement key ${key}`,
      );
    }
    if (min != null && max != null && min > max) {
      throw new BadRequestException(
        `Rule condition minimum cannot exceed maximum for ${key}`,
      );
    }
