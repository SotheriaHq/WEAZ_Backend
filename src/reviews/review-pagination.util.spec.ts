...
describe('review pagination utilities', () => {
    ...
    it('builds a composite most-helpful cursor filter using helpfulCount, createdAt, and id', () => {
        ...
        expect(buildReviewCursorWhere(ReviewSortOption.MOST_HELPFUL, cursor)).toEqual({
            OR: [
                { helpfulCount: { lt: 12 } },
    ...
    it('returns null for invalid cursor payloads', () => {
        expect(buildReviewCursorWhere(ReviewSortOption.NEWEST, 'not-a-valid-cursor')).toBeNull();
