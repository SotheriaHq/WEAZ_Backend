...
describe('completed-order review lifecycle', () => {
  ...
  beforeEach(() => {
    ...
    service = new ReviewsService(
      prisma as PrismaService,
      {
        enqueueProductAggregate: jest.fn(),
        enqueueBrandAggregate: jest.fn(),
      } as any,
      featureFlags as FeatureFlagsService,
  ...
  it('completed standard order creates product, collection, and brand prompts', async () => {
    ...
    const prompts =
      await eligibility.createPromptsForCompletedStandardOrder('order_1');
  ...
  it('completed custom design order creates design and brand prompts', async () => {
    ...
    const prompts =
      await eligibility.createPromptsForCompletedCustomOrder('custom_order_1');
  ...
  it('blocks edits after 24 hours but allows owner delete after 24 hours', async () => {
    ...
    prisma.review.update.mockResolvedValue(
      buildReview({ status: ReviewStatus.DELETED }),
    );
  ...
  it('does not let a brand user delete the buyer review', async () => {
    prisma.review.findUnique.mockResolvedValue(
      buildReview({ reviewerId: 'buyer_1' }),
    );
  ...
  it('validates rating 1-5 and required satisfaction', async () => {
    ...
    expect(errors.some((error) => error.property === 'satisfaction')).toBe(
      true,
    );
  });
  ...
  it('lets a brand report only its own lifecycle reviews without delete power', async () => {
    ...
    await expect(
      service.reportLifecycleReviewForBrand('brand_owner_1', 'review_1', {
        reason: 'SPAM' as any,
      }),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });
